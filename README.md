...
>
> **1 source → vibe audio (`.mp3`) + vibe reading (`.pptx`) + vibe knowledge base (`.md`)**
>
> **Multi-language output is built in** — the same source can be turned into notes, audio, and slides in English, Chinese, Japanese, Spanish, French, German, and more.
>
> The notebook stays local, the Markdown stays editable, and AI agents can keep using the folder after it is created.

```mermaid
flowchart LR
    A[1 source] --> B[1 local notebook folder]
    B --> C[Vibe KB .md]
    B --> D[Vibe audio .mp3]
    B --> E[Vibe reading .pptx]
    B --> F[AI agents can reuse it]
```
...
spoken recap, and the slide deck. The `.md` notebook is just a normal file,
so you can edit it yourself — and your agents can keep reading and building
on top of it. You can also ask yumnb to produce the notebook in different
output languages from the same source.
...
Here is a real slide generated by yumnb from a YouTube notebook request:

![Sample yumnb slide output](assets/sample-slide-agentic-engineering.jpg)

## At a glance

| If you want to… | yumnb gives you… |
| --- | --- |
| Build a local knowledge note from one source | `summary.md` |
| Get a vibe-listen version | `talkshow.txt` + `talkshow.mp3` |
| Get a vibe-reading version | `deck.json` + `deck.pptx` |
| Output in different languages | notes + audio + slides from the same source |
| Keep everything editable and inspectable | normal local files under one folder |
| Let agents continue working on it | a reusable local notebook folder |
...
| `summary.md` | The written notebook / local knowledge-base entry |
...
- **Multi-language output** — choose the output language for notes, talk-show
  script, TTS, and slides. Built-in default voice pairs ship for English,
  Chinese, Japanese, Spanish, French, and German; other languages can be added.
